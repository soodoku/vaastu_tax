#!/usr/bin/env python3
"""Collect Housing.com search result pages.
...
Example
-------
python scripts/housingcom/01_collect_search.py --city hyderabad --max-pages 10
python scripts/housingcom/01_collect_search.py --all-cities --max-pages 5
python scripts/housingcom/01_collect_search.py --city hyderabad --resume
python scripts/housingcom/01_collect_search.py --city hyderabad --retry-errors
"""
...
DOMAIN_HOUSINGCOM = "https://housing.com"
...
def parse_args() -> argparse.Namespace:
    ...
    city_group.add_argument(
        "--city", help="City key from data/config/housingcom_cities.json"
    )
    city_group.add_argument(
        "--all-cities",
        action="store_true",
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="Base directory for raw data. Defaults to data/raw/housingcom/",
    )
    parser.add_argument(
        "--max-pages",
        type=int,
    ...
    parser.add_argument(
        "--min-sleep", type=float, default=1.0, help="Minimum sleep between requests"
    )
    parser.add_argument(
        "--max-sleep", type=float, default=2.5, help="Maximum sleep between requests"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--headless",
        action="store_true",
        help="Run Chromium headless (default is headful)",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
...
def city_outdir_housingcom(city: str, base_dir: Path | None = None) -> Path:
    if base_dir:
        return base_dir / slugify(city)
    return project_root() / "data" / "raw" / "housingcom" / slugify(city)


def inventory_total_from_text(text: str) -> int | None:
...
def main() -> None:
    ...
    if args.all_cities:
        ...
        for city_key in cities_to_collect:
            ...
            outdir = city_outdir_housingcom(city_key, base_outdir)
            ensure_dir(outdir)
        ...
        outdir = city_outdir_housingcom(args.city, base_outdir)
        ensure_dir(outdir)
